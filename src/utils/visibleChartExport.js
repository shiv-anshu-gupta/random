...
export async function exportVisibleChartsAsComtrade({
  ...
  if (Array.isArray(data?.computedData)) {
    computedIds.forEach((computedId) => {
      const entry = data.computedData.find((item) => item && item.id === computedId);
      if (!entry) return;
      const values = coerceArray(entry.data);
      if (!values.length) return;
      analogSeries.push(
        resolveComputedSeries({
          computedEntry: { id: computedId, name: entry.name, unit: entry.unit, values },
          channelState,
        })
      );
    });
